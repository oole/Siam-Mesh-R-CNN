...
from tracker.tracking.fpn_lc_conv.fpn2mesh_lc_conv_argmax_tracker import ArgmaxConvTracker
from tracker.tracking.fpn_lc_conv.fpn2mesh_lc_conv_three_stage_tracker import ThreeStageConvTracker
...
parser.add_argument('--model', type=str, default="oole-resnet50-fpn2mesh_lc",
...
parser.add_argument('--computed_dir', default="../../computed/tracker")
...
def build_tracker():
    if args.tracker == "ArgmaxTracker":
        name = "FPN2Mesh-LC-" + args.tracker + "-" + args.model
        if args.path_to_model is not None:
            name += "-" + os.path.basename(args.path_to_model)
        return ArgmaxConvTracker(model=args.model, name=name,
    ...
    name = "FPN2Mesh-LC-ThreeStageTracker_" + param_str
