...
from tracker.tracking.got10 import tracking_dataset


class ExperimentChokepoint(object):
    r"""Experiment pipeline and evaluation toolkit for Chokepoint Dataset
    ...
    def __init__(self, name, result_dir='results', report_dir='reports', start_idx=0, end_idx=None):
        super(ExperimentChokepoint, self).__init__()
        self.dataset = tracking_dataset.Chokepoint()
        self.result_dir = os.path.join(result_dir, 'Chokepoint-' + str(name))
        self.report_dir = os.path.join(report_dir, 'Chokepoint-' + str(name))
        ...
        for s in range(self.start_idx, end_idx):
            ...
            if hasattr(self, "use_confs") and self.use_confs:
                boxes, times, confs = tracker.track(
                    img_files, anno[0, :], visualize=visualize, use_confidences=True, box_format='coordinates')
    ...
    def plot_curves(self, tracker_names):
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                legends.append(
                    '$\\bf{Siam}$ $\\bf{R}$$\\bf{-}$$\\bf{CNN}$: [%.3f]' % performance[name][key]['success_score'])
            else:
        ...
        for i, name in enumerate(tracker_names):
            ...
            if name == "Siam R-CNN":
                legends.append(
                    '$\\bf{Siam}$ $\\bf{R}$$\\bf{-}$$\\bf{CNN}$: [%.3f]' % performance[name][key]['precision_score'])
            else:
