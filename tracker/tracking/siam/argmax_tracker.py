...
class PrecomputingReferenceTracker(Tracker):
    def __init__(self, name, need_network=True, need_img=True, model="best"):
        ...
        if need_network:
            ...
            elif model == "oole-resnet50":
                load = "/home/oole/lc_conv-track/model-dir/face-siamrcnn_resnet50_220125_1902_eval/model-250000"
                cfg.BACKBONE.RESNET_NUM_BLOCKS = [3, 4, 6, 3]
            elif model == "best":
                # TODO Own model
                # For original pre-trained:
                load = "/home/oole/git/projects-to-consume/SiamR-CNN/train_log/hard_mining3/model-1360500"
                cfg.BACKBONE.RESNET_NUM_BLOCKS = [3, 4, 23, 3]  # resnet101
...
class ArgmaxTracker(PrecomputingReferenceTracker):
    def __init__(self, model):
        super().__init__("ArgmaxTracker", model=model)

    def _update(self, img):
        from tracker.util.eval_util import predict_image_track_with_precomputed_ref_features
