...
from tracker.model.siam_mesh.siam_mesh_rcnn_lc_model import ResNetFPNMeshModelLC
from tracker.util.data_util import CustomResize, box_to_point8, point8_to_box


class PrecomputingReferenceTracker(Tracker):
    def __init__(self, name, need_network=True, need_img=True, model="best", conv_mode='spiral'):
        ...
        if need_network:
            ...
            elif model == "oole-resnet50-lc_conv":
                load = "/home/oole/lc_conv-track/model-dir/partial_tp_conv-track_img2mesh2feature-lc_220130-0153/model-110000"
            cfg.BACKBONE.RESNET_NUM_BLOCKS = [3, 4, 6, 3]
        else:
        ...
        extract_model = ResNetFPNMeshModelLC(mesh_path=cfg.COMA.TEMPLATE_MESH_TRACKER, fpn_conv_mode=self.conv_mode,
...
class ArgmaxConvTracker(PrecomputingReferenceTracker):
    def __init__(self, model, conv_mode='spiral'):
        super().__init__("ArgmaxTracker", model=model, conv_mode=conv_mode)
