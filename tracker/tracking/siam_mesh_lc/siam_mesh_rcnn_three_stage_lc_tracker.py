...
from tracker.tracking.siam_mesh_lc.siam_mesh_rcnn_argmax_lc_tracker import PrecomputingReferenceTracker
from tracker.tracking.tracking_util import resize_and_clip_boxes, generate_colors, xyxy_to_cxcywh_np
...
class ThreeStageConvTracker(PrecomputingReferenceTracker):
    def __init__(self, tracklet_distance_threshold=0.06, tracklet_merging_threshold=0.3,
                 ...
                 name="ThreeStageTracker", model="best", n_proposals=None, resolution=None, conv_mode='spiral'):
        """
        ...
        super().__init__(name=name, need_network=True, need_img=True, model=model, conv_mode=conv_mode)
    ...
    def init(self, image, box):
        ...
        # tracklet also contains mesh information, so _ff_gt_feats must also contain mesh features which come as separate output
        ff_gt_all_feats = np.concatenate((self._ff_gt_feats, self._ff_gt_mesh_feats), axis=0)
        self._ff_gt_tracklet.add_detection(ff_gt_all_feats, self._ff_box, 1.0, 1.0)
        self._all_tracklets = [self._ff_gt_tracklet]

    def _make_pred_func(self, load):
        cfg.MODE_THIRD_STAGE = True
        from tracker.model.siam_mesh.siam_mesh_rcnn_lc_model import MeshTrackModelLC
        pred_model = MeshTrackModelLC(mesh_path=cfg.COMA.TEMPLATE_MESH_TRACKER,
                                      fpn_conv_mode=self.conv_mode,
                                      coma_conv_mode=self.conv_mode)
    ...
    def _update_tracklets(self, img):
        ...
        tracklet_score_indices = self._pred_func(
            resized_img, self._ff_gt_feats, self._ff_gt_mesh_feats, self._ff_gt_tracklet.feats[-1],
            active_tracklets_feats,
        ...
        for det_idx in range(n_dets):
            ...
            if tracklet_scores.size > 0:
                if tracklet_scores[:, det_idx].max() > self._tracklet_merging_threshold:
                    ...
                    if (tracklet_scores[
                            tracklet_idx] >= max_score - self._tracklet_merging_second_best_relative_threshold).sum() == 1:
                        # there should be no other tracklet to which this det is similar...
                        if (tracklet_scores[:,
                            det_idx] >= max_score - self._tracklet_merging_second_best_relative_threshold).sum() == 1:
    ...
    def _track(self):
        ...
        if len(active_tracklets) > 0:
            ...
            all_pairwise_diffs = np.abs(
                active_start_boxes_cxcywh[:, np.newaxis] - alive_end_boxes_cxcywh[np.newaxis]) / norm
