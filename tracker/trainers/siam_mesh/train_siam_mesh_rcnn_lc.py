...
from tracker.model.siam_mesh import siam_mesh_rcnn_lc_model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--logdir', help='log directory', default='../../computed/siam_mesh_rcnn_lc')
    parser.add_argument('--load', nargs="+",
                        help='load a model for evaluation or training. Can overwrite BACKBONE.WEIGHTS')
    parser.add_argument('--fpn-conv-mode', help="The conv mode, either 'cheb' or 'spiral'", default="spiral")
    parser.add_argument('--coma-conv-mode', help="The conv mode, either 'cheb' or 'spiral'", default="spiral")

    args = parser.parse_args()

    # tracker_model = track_model.ResNetFPNTrackModel()
    tracker_model = siam_mesh_rcnn_lc_model.MeshTrackModelLC(mesh_path=cfg.COMA.TEMPLATE_MESH_TRACKER,
                                                        fpn_conv_mode=args.fpn_conv_mode,
                                                        coma_conv_mode=args.coma_conv_mode)
    ...
    callbacks = [
                    PeriodicCallback(
                        ModelSaver(max_to_keep=30, keep_checkpoint_every_n_hours=1),
                        # every_k_epochs=1),
                        every_k_epochs=10),
                    # linear warmup
    ...
    max_epoch = cfg.TRAIN.MAX_NUM_EPOCHS  # min(cfg.TRAIN.LR_SCHEDULE[-1] * factor // stepnum, cfg.TRAIN.MAX_NUM_EPOCHS)
