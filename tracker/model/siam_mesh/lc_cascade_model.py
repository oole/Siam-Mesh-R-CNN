...
from tracker.model.siam import box_model, frcnn_model
# from model_box import clip_boxes
from tracker.model.util.box_ops import pairwise_iou


class LCCascadeRCNNHead(object):
    def __init__(self, proposals,
        ...
        with tf.variable_scope('cascade_rcnn_stage3'):
            ...
            H3, B3 = self.run_head(B2_proposal, 2, compute_mesh_features=True)
        self._cascade_boxes = [B1, B2, B3]
        self._heads = [H1, H2, H3]

    def run_head(self, proposals, stage, compute_mesh_features=False):
        """
        Args:
            proposals: BoxProposals
            stage: 0, 1, 2
        ...
        pooled_feature = self.roi_func(proposals.boxes, compute_mesh_features=compute_mesh_features)  # N,C,S,S
        pooled_feature = self.scale_gradient(pooled_feature)
        head_feature = self.fastrcnn_head_func('head', pooled_feature)
        # changed by Paul
        label_logits, box_logits = frcnn_model.fastrcnn_outputs(
            'outputs_new', head_feature, self.num_classes, class_agnostic_regression=True)
        head = frcnn_model.FastRCNNHead(proposals, box_logits, label_logits, self.gt_boxes, reg_weights)

        refined_boxes = head.decoded_output_boxes_class_agnostic()
        refined_boxes = box_model.clip_boxes(refined_boxes, self.image_shape2d)
    ...
    def match_box_with_gt(self, boxes, iou_threshold):
        ...
        if self.is_training:
            with tf.name_scope('match_box_with_gt_{}'.format(iou_threshold)):
                ...
                return frcnn_model.BoxProposals(boxes, labels_per_box, fg_inds_wrt_gt)
        else:
            return frcnn_model.BoxProposals(boxes)
