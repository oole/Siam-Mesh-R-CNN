...
from tensorpack.models import Conv2D, MaxPooling, layer_register, FixedUnPooling
from tensorpack.tfutils import tower
from tensorpack.tfutils.argscope import argscope
from tensorpack.tfutils.scope_utils import under_name_scope
from tensorpack.tfutils.summary import add_moving_summary

from tracker.config import config as cfg
from tracker.model.siam import box_model, rpn_model
from tracker.model.siam.base_model import GroupNorm
from tracker.model.util import box_ops
...
def multilevel_roi_align(features, rcnn_boxes, resolution):
    ...
    for i, boxes, featuremap in zip(itertools.count(), level_boxes, features):
        with tf.name_scope('roi_level{}'.format(i + 2)):
            ...
            all_rois.append(box_model.roi_align(featuremap, boxes_on_featuremap, resolution))
...
@under_name_scope()
def multilevel_roi_align_with_meshes(image, features, rcnn_boxes, resolution, mesh_helper):
    """
    Args:
        features ([tf.Tensor]): 4 FPN feature level 2-5
        rcnn_boxes (tf.Tensor): nx4 boxes
    ...
    for i, boxes, featuremap in zip(itertools.count(), level_boxes, features):
        with tf.name_scope('roi_level{}'.format(i + 2)):
            boxes_on_featuremap = boxes * (1.0 / cfg.FPN.ANCHOR_STRIDES[i])
            all_rois.append(
                box_model.roi_align_with_meshes(image, boxes, featuremap, boxes_on_featuremap, resolution, mesh_helper))
...
def fpn_map_rois_to_levels(boxes):
    ...
    """
    sqrtarea = tf.sqrt(box_ops.area(boxes))
    level = tf.cast(tf.floor(
        4 + tf.log(sqrtarea * (1. / 224) + 1e-6) * (1.0 / np.log(2))), tf.int32)  # TODO could use tf.log(2)
...
def generate_fpn_proposals(
    ...
    training = tower.get_current_tower_context().is_training
    all_boxes = []
    all_scores = []
    if cfg.FPN.PROPOSAL_MODE == 'Level':
        ...
        for lvl in range(num_lvl):
            with tf.name_scope('Lvl{}'.format(lvl + 2)):
                ...
                proposal_boxes, proposal_scores = rpn_model.generate_rpn_proposals(
...
def multilevel_rpn_losses(
    ...
    with tf.name_scope('rpn_losses'):
        for lvl in range(num_lvl):
            ...
            label_loss, box_loss = rpn_model.rpn_losses(
