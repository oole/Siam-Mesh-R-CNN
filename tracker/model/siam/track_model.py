...
from tracker.model.siam import base_model, box_model, cascade_model, fpn_model, frcnn_model, rpn_model, mrcnn_model
...
class DetectionModel(ModelDesc):
    def preprocess(self, image):
        ...
        image = base_model.image_preprocess(image, bgr=True)
...
class ResNetFPNModel(DetectionModel):
    ...
    def roi_heads(self, image, features, proposals, targets):
        ...
        if not cfg.FPN.CASCADE:
            ...
            def roi_func(boxes):
                return fpn_model.multilevel_roi_align(features[:4], boxes, 7)
        ...
        if self.training:
            ...
            if cfg.MODE_MASK:
                gt_masks = targets[2]
                # maskrcnn loss
                roi_feature_maskrcnn = fpn_model.multilevel_roi_align(
                    features[:4], proposals.fg_boxes(), 14,
                    name_scope='multilevel_roi_align_mask')
                maskrcnn_head_func = getattr(mrcnn_model, cfg.FPN.MRCNN_HEAD_FUNC)
                mask_logits = maskrcnn_head_func(
                    'maskrcnn', roi_feature_maskrcnn, cfg.DATA.NUM_CATEGORY)  # #fg x #cat x 28 x 28

                target_masks_for_fg = box_model.crop_and_resize(
                    tf.expand_dims(gt_masks, 1),
                    proposals.fg_boxes(),
                    proposals.fg_inds_wrt_gt, 28,
                    pad_border=False)  # fg x 1x28x28
                target_masks_for_fg = tf.squeeze(target_masks_for_fg, 1, 'sampled_fg_mask_targets')
                all_losses.append(mrcnn_model.maskrcnn_loss(mask_logits, proposals.fg_labels(), target_masks_for_fg))
            return all_losses
        else:
            ...
            if cfg.MODE_MASK:
                ...
                roi_feature_maskrcnn = fpn_model.multilevel_roi_align(features[:4], final_boxes, 14)
                maskrcnn_head_func = getattr(mrcnn_model, cfg.FPN.MRCNN_HEAD_FUNC)
                mask_logits = maskrcnn_head_func(
                    'maskrcnn', roi_feature_maskrcnn, cfg.DATA.NUM_CATEGORY)  # #fg x #cat x 28 x 28
...
class ResNetFPNTrackModel(ResNetFPNModel):
    ...
    def roi_heads(self, image, ref_features, ref_box, features, proposals, targets, precomputed_ref_features=None):
        image_shape2d = tf.shape(image)[2:]  # h,w
        assert len(features) == 5, "Features have to be P23456!"
        gt_boxes, gt_labels, *_ = targets
