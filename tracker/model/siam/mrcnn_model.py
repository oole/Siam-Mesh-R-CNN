...
from tensorpack.models import layer_register, Conv2D, Conv2DTranspose
...
def maskrcnn_upXconv_head(feature, num_category, num_convs, norm=None):
    ...
    with argscope([Conv2D, Conv2DTranspose], data_format='channels_first',
                  kernel_initializer=tf.variance_scaling_initializer(
                      ...
                      distribution='untruncated_normal' if common_util.get_tf_version_tuple() >= (1, 12) else 'normal')):
        # c2's MSRAFill is fan_out
        for k in range(num_convs):
            l = Conv2D('fcn{}'.format(k), l, cfg.MRCNN.HEAD_DIM, 3, activation=tf.nn.relu)
            if norm is not None:
                l = backbone.GroupNorm('gn{}'.format(k), l)
