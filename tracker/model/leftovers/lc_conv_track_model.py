...
from tracker.model.siam_mesh import lc_cascade_model
from tracker.model.coma import mesh_model
from tracker.model.siam import base_model, box_model, fpn_model, frcnn_model, rpn_model
...
class ResNetFPNConvModel(DetectionModel):
    """
    Since conv_track_model is so expensive to compute, only do it on the last cascade of the FRCNN header.
    """
    ...
    def get_mesh_features(self, image, boxes, box_ind, crop_size, pad_border=True):
        ...
        def transform_bboxes_for_crop_on_image(boxes, image_shape, crop_shape):
            ...
            imshape = [tf.cast(image_shape[0] - 1, tf.float32),
                       tf.cast(image_shape[1] - 1, tf.float32)]  # shape[0] = 600+crop size, shape[1] = 800 + crop size
        ...
        mesh_features = self.mesh_helper.img2features(crops)
    ...
    def roi_heads(self, image, features, proposals, targets):
        ...
        if not cfg.FPN.CASCADE:
            ...
            def roi_func(boxes, compute_mesh_features=False):
                aligned_features = fpn_model.multilevel_roi_align(features[:4], boxes, 7)
                if compute_mesh_features:
                    target_mesh_features = self.get_mesh_features(image, boxes,
                                                                  tf.zeros([tf.shape(boxes)[0]]), 224)

                    aligned_features = tf.concat((aligned_features, target_mesh_features), axis=1)
                return aligned_features

            fastrcnn_head = lc_cascade_model.LCCascadeRCNNHead(
                proposals, roi_func, fastrcnn_head_func,
                (gt_boxes, gt_labels), image_shape2d, cfg.DATA.NUM_CLASS)

        if cfg.EXTRACT_GT_FEATURES:
            roi_feature_fastrcnn = fpn_model.multilevel_roi_align(features[:4], proposals.boxes, 7)
            roi_feature_fastrccn_mesh = self.get_mesh_features(image, proposals.boxes,
                                                               tf.zeros([tf.shape(proposals.boxes)[0]]), 224)
...
class ConvTrackModel(ResNetFPNConvModel):
    def __init__(self, mesh_path=None):
    ...
    def roi_heads(self, image, ref_image, ref_features, ref_box, features, proposals, targets,
        ...
        if precomputed_ref_features is None:
            ...
            ref_mesh_features = self.get_mesh_features(ref_image, ref_box[tf.newaxis],
                                                       tf.zeros([tf.shape(ref_box[tf.newaxis])[0]]), 224)
        ...
        def roi_func(boxes, already_aligned_features=None, compute_mesh_features=False):
            ...
            if compute_mesh_features:
                ...
                tiled = tf.tile(roi_aligned_ref_features, [tf.shape(all_features)[0], 1, 1, 1])
                # for each box the ref_features are concatenated with the aligned features
                concat_features = tf.concat((tiled, all_features), axis=1)
    ...
    def build_graph(self, *inputs):
        ...
        if cfg.MODE_THIRD_STAGE:
            self._run_third_stage(image, inputs, second_stage_features, tf.shape(image)[2:4])
    ...
    def _run_third_stage(self, image, inputs, second_stage_features, image_hw):
        ...
        mesh_features_curr = self.get_mesh_features(image, boxes, tf.zeros([tf.shape(boxes)[0]]), 224)
    ...
    def _score_for_third_stage(self, ref_feats, det_feats, dense=True, ref_boxes=None, det_boxes=None, image_hw=None,
        ...
        ref_feats_no_mesh = ref_feats[:, :-1]
        det_feats_no_mesh = det_feats[:, :-1]
        ...
        active_tracklets_tiled_no_mesh = tf.tile(ref_feats_no_mesh[:, tf.newaxis], multiples=[1, n_dets, 1, 1, 1])
        dets_tiled_no_mesh = tf.tile(det_feats_no_mesh[tf.newaxis], multiples=[n_refs, 1, 1, 1, 1])

        concated_no_mesh = tf.concat([active_tracklets_tiled_no_mesh, dets_tiled_no_mesh], axis=2)
        ...
        if not dense:
            ...
            flattened_no_mesh = tf.boolean_mask(concated_no_mesh, keep_mask)
        else:
            indices = None
            flattened = tf.reshape(
                concated, [tf.shape(concated)[0] * tf.shape(concated)[1]] + [int(x) for x in concated.shape[2:]])
            flattened_no_mesh = tf.reshape(
                concated_no_mesh, [tf.shape(concated_no_mesh)[0] * tf.shape(concated_no_mesh)[1]] +
                                  [int(x) for x in concated_no_mesh.shape[2:]])
                ...
                with argscope(Conv2D, data_format='channels_first'):
                    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
                        if idx == 2:
                            ...
                            reduced_features = Conv2D('conv_reduce', flattened_no_mesh, 256, 1, activation=None)
